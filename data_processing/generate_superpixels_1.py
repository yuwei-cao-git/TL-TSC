...
def generate_superpixels(tile_path, size_threshold, shapefile, output_dir):
    ...
    nodata_mask = (
        np.any(tile_image == nodata_value, axis=0)
        if nodata_value is not None
        else np.zeros((tile_height, tile_width), dtype=bool)
    )
    ...
    intersecting_polygons["geometry"] = intersecting_polygons.geometry.map(
        lambda geom: geom.intersection(tile_bbox)
    )
    ...
    polygon_id_to_label = dict(
        zip(intersecting_polygons["POLYID"], intersecting_polygons["perc_specs"])
    )
    ...
    small_sp_ids = [
        sp_id for sp_id, count in zip(sp_ids, counts) if count < size_threshold
    ]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate superpixels for satellite tiles"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=25,
        help="Minimum number of pixels in a superpixel",
    )
    parser.add_argument(
        "--workers",
        type=int,
...
if __name__ == "__main__":
    ...
    # Input/output paths based on dataset
    shapefile = f"/mnt/d/Sync/research/tree_species_estimation/tree_dataset/{dataset}/processed/{dataset}_fri/{dataset}_fri_superpixel_updated.gpkg"
    ...
    for season in seasons:
        ...
        output_dir = f"/mnt/d/Sync/research/tree_species_estimation/tree_dataset/{dataset}/processed/{dataset}_s2/{season}/superpixel"
        os.makedirs(output_dir, exist_ok=True)

        tile_paths = glob.glob(os.path.join(tile_dir, "*.tif"))
        print(f"\nProcessing {len(tile_paths)} tiles for season: {season}")

        results = Parallel(n_jobs=args.workers)(
            delayed(generate_superpixels)(
                tile_path, size_threshold, shapefile, output_dir
            )
